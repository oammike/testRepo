<?php namespace OAMPI_Eval\Http\Controllers;
                     ...
                     // $doneEval = new Collection;
                      ...
                      if ($this->user->userType_id !== 4 && !($leadershipcheck->isEmpty())) //if not AGENT
                      {
                            ...
                            /* ---------------------------------------------------------------- */
                                ...
                                $data = $this->getPastMemberEvals($mc, $evalSetting);
                                //return $data;
...
/* -------  for maxscore -----*/
    ...
    public function show($id)
    {
        ...
        if( $this->user->userType_id == 1 || $this->user->userType_id == 5 ||  $allowed || $this->user->id == $employee->id ){
              ...
              } else
              {
                  ...
                  return view('evaluation.show-employee', compact('updateStatus', 'allowed', 'doneEval', 'evaluator', 'startPeriod', 'endPeriod', 'performanceSummary', 'evalType', 'employee', 'ratingScale', 'evalForm','evalSetting', 'formEntries','maxScore','summaries'));
              }        
    ...
    public function store(Request $request)
    {
        ...
        if (!$evalForm->isEmpty)
        {
            ...
            /* -------------- log updates made --------------------- */
         $file = fopen('public/build/changes.txt', 'a') or die("Unable to open logs");
            fwrite($file, "-------------------\n EvalID: ". $evalForm->id ." for: ". $employee->lastname.", ". $employee->firstname." newly added ". date('M d h:i:s'). " by ". $this->user->firstname.", ".$this->user->lastname."\n");
            //fclose($file);
