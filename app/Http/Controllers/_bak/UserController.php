...
class UserController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('people.employee-index', compact('TLs', 'myCampaign', 'users', 'hasUserAccess'));
                ...
                if ($emp->status_id !== 7 && $emp->status_id !== 8 && $emp->status_id !== 9){
                        ...
                        if ($this->user->employeeNumber !== $emp->employeeNumber)
                        {
                            ...
                            if (!is_null($isTL)){
                                ...
                                $mySubordinates->push(['id'=>$emp->id, 'isLeader'=>true, 'lastname'=> $emp->lastname, 'firstname'=>$emp->firstname, 'position'=>$emp->position->name, 'subordinates'=>$activeMen, 'completedEvals'=>$completedEvals ]);

                            } 
                            else {
                                $mySubordinates->push(['id'=>$emp->id, 'isLeader'=>false, 'lastname'=> $emp->lastname, 'firstname'=>$emp->firstname, 'position'=>$emp->position->name, 'subordinates'=>null, 'completedEvals'=>null ]);
                            }
    ...
    public function show($id)
    {
        ...
        || $this->user->employeeNumber==$leader_L0->employeeNumber || $this->user->employeeNumber==$leader_PM->employeeNumber  )  //($this->user->userType_id == 1 || $this->user->userType_id == 2)
        {
           ...
           // $immediateHead = ImmediateHead::find(ImmediateHead_Campaign::find($user->team->immediateHead_Campaigns_id)->immediateHead_id);
            ...
            //return $workSchedule;
            return view('people.show', compact('user','immediateHead','canCWS','canChangeSched', 'canMoveEmployees', 'canEditEmployees', 'camps','workSchedule'));
