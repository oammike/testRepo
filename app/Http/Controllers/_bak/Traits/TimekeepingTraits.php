...
trait TimekeepingTraits
{
  ...
  public function getWorkedHours($userLogIN, $userLogOUT, $schedForToday,$shiftEnd, $payday)
  {
    ...
    $OTattribute = "";
        ...
        if (count($userLogIN[0]['logs']) > 0 && count($userLogOUT[0]['logs']) > 0)
        {
          ...
          if ($userLogIN[0]['timing']->format('H:i:s') > $schedForToday['timeStart'])  $isLateIN = true; else $isLateIN= false;
          ...
          if ($isEarlyOUT && $isLateIN)//use user's logs
          {
            ...
            else { $workedHours = number_format($wh/60,2); $billableForOT=0; }
  ...
  public function getComplicatedWorkedHours($userLogIN, $userLogOUT, $schedForToday,$shiftEnd,$isRDYest,$payday)
  {
    ...
    $billableForOT=0; $endshift = Carbon::parse($shiftEnd); $diff = null; $OTattribute="";
        ...
        if (count($userLogIN[0]['logs']) > 0 && count($userLogOUT[0]['logs']) > 0)
        {
          ...
          if ($isRDYest)
          {
            ...
            if ($actualIN > $todayStart && $actualIN < $todayEnd) $isLateIN=true; else $isLateIN=false;
            if ($actualOUT > $todayStart && $actualOUT < $todayEnd) $isEarlyOUT=true; else $isEarlyOUT=false;
